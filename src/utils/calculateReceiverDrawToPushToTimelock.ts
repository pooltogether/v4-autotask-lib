...
export async function calculateReceiverDrawToPushToTimelock(
  ...
  try {
    ...
    const {
      drawId: drawIdNewestFromBeaconChain,
    ...
    const {
      drawId: drawIdOldestFromBeaconChain,
  ...
  debug(
    `ReceiverChain: Newest PrizeDistribution Draw ID is ${newestDrawIdFromReceiverChain}`
  );
  ...
  if (oldestDrawIdFromBeaconChain === 0 && newestDrawIdFromBeaconChain === 0) {
    throw new Error(
      'BeaconChainPrizeDistributionBuffer/no-prize-distribution-buffer-available'
    );
  }
