...
export async function generateYieldForPrizePool(
  contracts: ContractsBlob,
  config: Config,
  relayer: Relayer
): Promise<ActionState> {
  const provider = getInfuraProvider(config.network, config.apiKey);
  const mockYieldSource = getContract(
    'MockYieldSource',
    config.chainId,
    provider,
    contracts
  ...
  const depositToken = new ethers.Contract(
    await mockYieldSource.getToken(),
    ERC20,
    provider
  );
  ...
  try {
    ...
    const currentBalance = await depositToken.balanceOf(
    ...
    debug(
      `Total ${
        config.network
      } mock yield source deposits is ${ethers.utils.formatUnits(
        currentBalance,
        decimals
      )}`
    );
    debug(
      `Yielding ${ethers.utils.formatUnits(yieldTokens, decimals)} on ${
        config.network
      }...`
    );
