...
export async function getDrawCalculatorTimelockState(
  contracts: ContractsBlob,
  config: Config
): Promise<IDrawCalculatorTimelockState> {
  try {
    const provider = getJsonRpcProvider(
      `https://${config.network}.infura.io/v3/${config.apiKey}`
    );
    ...
    if (!DrawCalculatorTimelock)
      throw new Error('DrawCalculatorTimelock not found');
